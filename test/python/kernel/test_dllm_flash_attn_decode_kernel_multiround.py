...
def test_decode_multiround_context_len():
    ...
    for num_seqs in num_seqs_list:
        ...
        for context_len in context_lens:
            ...
            context_lens_tensor = torch.full((num_seqs,), context_len, dtype=torch.int32, device=device)
            cu_seqlens_q = torch.arange(
                0,
                (num_seqs + 1) * base_params["diffusion_block_size"],
                base_params["diffusion_block_size"],
                dtype=torch.int32,
                device=device,
            )
    ...
    print("=" * 80)
    print(
        f"{'Num Seqs':<12} {'Context Len':<15} {'Compiled':<10} {'Correct':<10} {'Compilation Time (s)':<20} {'Inference Time (ms)':<20}"
    )
    print("-" * 100)
    for num_seqs in num_seqs_list:
        for context_len in context_lens:
    ...
    if len(unique_kernel_paths) == total_combinations:
        ...
        print(
            f"⚠ Some combinations shared the same kernel ({len(unique_kernel_paths)} unique kernels for {total_combinations} combinations)"
        )
    ...
    if passed < total:
        print("  Failed (num_seqs, context_len) combinations:")
