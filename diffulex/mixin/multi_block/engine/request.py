...
class DllmReqMultiBlockMixin:
    def init_multi_block(self: DllmReq, config: Config):
        ...
        self.diffusion_block_size = config.diffusion_block_size  # alias for scheduler
        ...
        for i in range(self.padded_prefix_len // self.block_size):
            ...
            self.dllm_blocks[-1].make_in_context()
    ...
    def eos_token_generated(self) -> bool:
        ...
        return eos_detect_fn(seq) or (last_in_cache_block.is_last_in_context and eos_detect_fn(last_in_cache_block.token_ids))
    ...
    def running_len(self) -> int:
        if self.is_prefilling:
            return (
                ...
                if self.is_padded else self.prefix_len
            )
    ...
    def cache_len(self) -> int:
        return self.in_cache_len + self.to_cache_len
    ...
    def step(self):
        ...
        activate_cond_all_dummy_buffer_backup = (
            not self.dllm_block_buffer.active_blocks and 
            not self.eos_token_generated and
            self.dllm_block_buffer.dllm_blocks[0].is_dummy and 
            self.dllm_block_buffer.dllm_blocks[0].prev_block.is_in_cache
        )
    ...
    def push_back_dummy_block(self):
        ...
        if (self.max_new_tokens_reached or self.max_model_len_reached) and new_block.prev_block.is_in_context:
            new_block.make_last_in_context()
        elif new_block.prev_block.is_out_of_context or new_block.prev_block.is_last_in_context:
            new_block.make_out_of_context()
        
        self.dllm_blocks.append(new_block)
        self.dllm_block_buffer.push_back(new_block)
    ...
    def postprocess(self):
        ...
        if (self.eos_token_generated or self.max_new_tokens_reached or self.max_model_len_reached) and self.last_block_finished:
