...
def ensure_scale_tensor(
    ...
    *,
    ...
    """Returns a CUDA tensor suitable for Triton: shape [num_kv_heads], dtype float32 by default."""
    if scale is None:
...
def view_fp8_cache(cache: torch.Tensor, kv_cache_dtype: str) -> torch.Tensor:
    """FP8 KV cache uses uint8 as storage; returns a view with fp8 dtype for kernels."""
