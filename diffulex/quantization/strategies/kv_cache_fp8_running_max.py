...
from diffulex.quantization.kv_cache_dtype import parse_kv_cache_dtype, view_fp8_cache
from diffulex.quantization.registry import register_kv_cache_strategy
...
class kv_cacheFP8RunningMaxStrategy(KVCacheQuantizationStrategy):
    ...
    def quantize(self, tensor: torch.Tensor, scale: torch.Tensor, **kwargs) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        quantized = (tensor.float() * descale).clamp(min=float(self.spec.fp8_min), max=float(self.spec.fp8_max))
