...
from diffulex.strategy.multi_block_diffusion.attention.metadata import fetch_multi_bd_attn_metadata, set_multi_bd_attn_metadata, reset_multi_bd_attn_metadata


@AutoModelRunner.register("multi_block_diffusion", is_default=True)
class MultiBDModelRunner(ModelRunnerBase):
    ...
    def __init__(self, config: Config, rank: int, event: Event | list[Event]):
        ...
        self.init_attn_metadata_fn(set_multi_bd_attn_metadata, reset_multi_bd_attn_metadata, fetch_multi_bd_attn_metadata)
    ...
    def prepare_prefill(self, reqs: list[DllmReq]):
        self.prepare_prefill_multi_block(reqs)
    ...
    def run_model(self, input_ids: torch.Tensor, positions: torch.Tensor, is_prefill: bool):
        self.run_model_multi_block(input_ids, positions, is_prefill)

    def run(self, reqs: list[DllmReq], is_prefill: bool) -> list[int]:
        return self.run_multi_block(reqs, is_prefill)
