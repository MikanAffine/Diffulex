...
class FDV2Block:
    block_id: int = 0
    status: FDV2BlockStatus = FDV2BlockStatus.ACTIVE
    ...
    def to_cache(self) -> None:
        if self.available_to_cache and not self.is_in_cache:
            self.status = FDV2BlockStatus.TO_CACHE
    ...
    def is_active(self) -> bool:
        return self.status == FDV2BlockStatus.ACTIVE
    
    @property
    def is_to_cache(self) -> bool:
        return self.status == FDV2BlockStatus.TO_CACHE
    
    @property
    def is_in_cache(self) -> bool:
        return self.status == FDV2BlockStatus.IN_CACHE
...
class FDV2Req(DllmReq):
    ...
    @property
    def diffusion_block_status(self) -> list[FDV2BlockStatus]:
    ...
    def init_diffusion_blocks(self) -> None:
        ...
        for block_id in range(total_num_blocks):
            ...
            if block_id < num_prefix_blocks:
                status = FDV2BlockStatus.TO_CACHE
            elif is_last_prefix_block:
                status = FDV2BlockStatus.ACTIVE if has_mask_token else FDV2BlockStatus.TO_CACHE
            else:
                status = FDV2BlockStatus.TO_CACHE
