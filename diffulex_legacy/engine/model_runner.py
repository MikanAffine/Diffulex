...
from diffulex_legacy.models.auto_model import AutoModelLM
from diffulex_legacy.layers.sampler import AutoSampler
...
class ModelRunnerForCausalLM(ModelRunnerBase):
    ...
    def allocate_kv_cache(self):
        ...
        config.num_kv_cache_blocks = int(total * config.gpu_memory_utilization - used - peak + current) // block_bytes
...
class ModelRunnerForDiffusionLM(ModelRunnerBase):
    ...
    def allocate_kv_cache(self):
        ...
        current = torch.cuda.memory_stats()["allocated_bytes.all.current"]
        num_kv_heads = (
            getattr(hf_config, "num_key_value_heads", getattr(hf_config, "n_kv_heads", None)) // self.world_size
        )
        ...
        block_bytes = 2 * hf_config.num_hidden_layers * self.block_size * num_kv_heads * head_dim * itemsize
        get_num_kv_cache_blocks = (
            lambda gpu_memory_utilization: int(
                total * gpu_memory_utilization  # noqa: E731
                - used
                - peak
                + current
            )
        ...
        try:
            ...
            while num_kv_cache_blocks <= 200:
                print(
                    f"Warning: GPU memory utilization {gpu_memory_utilization} is too low to allocate kv cache. "
        ...
        print(
            f"Allocated {config.num_kv_cache_blocks} blocks of size {self.block_size} for kv cache on rank {self.rank}."
        )
        ...
        if config.kv_cache_layout == "distinct":
            ...
            raise ValueError(
                f"Unsupported kv_cache_layout: {config.kv_cache_layout}. Supported values are 'distinct' and 'unified'."
            )
            ...
            for mem_block_idx in range(0, seq.num_blocks):
                ...
                while start_idx < end_idx and not is_last_block and not meet_active_block:
                    ...
                    if diffusion_block.is_in_cache:
                        ...
                        start_idx += step
                        mem_block_start = (
                            seq.block_table[mem_block_idx] * self.block_size + context_len % seq.block_size
                        )
                        context_len += step
                        slot_mapping.extend(
                            list(
                ...
                if meet_active_block:
                    ...
                    break
            assert len(input_ids) == len(positions), (
                f"Input IDs length {len(input_ids)} does not match positions length {len(positions)}"
            )
            assert len(input_ids) == len(slot_mapping), (
                f"Input IDs length {len(input_ids)} does not match slot mapping length {len(slot_mapping)}"
            )
