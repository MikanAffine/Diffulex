...
from diffulex_legacy.sampling_params import SamplingParams
...
class SequenceForCausalLM(SequenceBase):
    ...
    def __repr__(self) -> str:
        return (
            f"SequenceForCausalLM(block_size={self.block_size}, counter={self.counter}, "
...
class DiffusionBlock:
    ...
    def current_complete_ratio(self) -> float:
        return (
            (sum([token_id != self.mask_token_id for token_id in self.token_ids]) / self.size) if self.size > 0 else 0.0
        )
...
class SequenceForDiffusionLM(SequenceBase):
    ...
    def __repr__(self) -> str:
        return (
            f"SequenceForDiffusionLM(block_size={self.block_size}, counter={self.counter}, "
            f"seq_id={self.seq_id}, status={self.status.name}, num_tokens={self.num_tokens}, "
            f"num_prompt_tokens={self.num_prompt_tokens}, num_cached_tokens={self.num_cached_tokens}, "
            f"temperature={self.temperature}, max_tokens={self.max_tokens}, ignore_eos={self.ignore_eos}, "
    ...
    def mem_block_to_diffusion_blocks_map(self) -> List[List[int]]:
        ...
        for block_id in range(self.num_blocks):
            ...
            mapping.append(
                [
                    self.token_to_diffusion_block_id(token_id)
                    for token_id in range(window_start, window_start + window_length)
                ]
            )  # build up token-wise mapping
        return mapping
    ...
    def update_block_mask(self, is_prefill: bool = False) -> None:
        if is_prefill:
            ...
            num_diffusion_blocks = (
                num_tokens - self.input_num_tokens + self.diffusion_block_size - 1
