...
from diffulex_legacy.layers.attention.ops import causal_lm_flash_decoding
...
@triton.jit
def load_kv_cache_kernel_kv_both(
    k_cache_ptr,
    v_cache_ptr,
    k_new_ptr,
    v_new_ptr,
    ...
    if global_blk_idx != -1:
        ...
        kv_cache_to_out_offsets = (  # [Seq, Hkv, Hdim]
            (cur_kv_cache_to_out_start_idx + kv_cache_seq_offsets[:, None])
        ...
        if inseq_blk_idx == 0:
            ...
            for diff_blk_idx in range(active_seq_len // DIFFUSION_BLOCK_SIZE):
                ...
                cur_kv_new_to_out_offsets = (  # [Seq, Hkv, Hdim]
                    (cur_kv_new_to_out_start_idx + kv_new_seq_offsets[:, None])
                    * kv_out_stride_s  # Seq: TokenIds over Offset
                    + head_idx * kv_out_stride_h  # Hkv: HeadId
