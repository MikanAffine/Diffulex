...
class Attention(nn.Module):
    ...
    def _update_and_compute_q_fp8_scale(
        self, q: torch.Tensor, kv_cache_dtype: str, device: torch.device
    ) -> torch.Tensor | None:
        """
        ...
        if context.is_prefill:
            ...
            if self.model_type == "causal_lm":
                ...
                if is_unified_layout:
                    ...
                    if spec.is_fp8:
                        from diffulex_legacy.layers.attention.ops.triton_flash_attention import (
                        ...
                        # Reshape output: [total_tokens, num_heads, head_dim] -> [1, num_heads, total_tokens, head_dim]
                        o = rearrange(o, "s h d -> 1 h s d").contiguous()
                    else:
                    ...
                    diffusion_lm_parallel_flash_decoding(
                        ...
                        context.cu_seqlens_q,
                        context.total_lens,
                        max(context.total_lens),
                        max(context.seq_lens),
                        1.0,
                        1.0,
