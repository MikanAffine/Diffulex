    ...
    for seq_idx, seq in enumerate(context.seqs):
        ...
        for local_mem_blk_idx, global_mem_blk_idx in enumerate(context.block_tables[seq_idx]):
            ...
            k_cache_list.append(
                k_cache[
                    global_mem_blk_idx,
                    cur_start_idx : cur_start_idx + remain_num_tokens,
                ]
            )
            v_cache_list.append(
                v_cache[
        ...
        assert torch.allclose(k_cache_temp, k_list[seq_idx][: seq.caching_num_tokens], atol=1e-5), (
            f"K cache mismatch for seq {seq_idx}!"
        )
        assert torch.allclose(v_cache_temp, v_list[seq_idx][: seq.caching_num_tokens], atol=1e-5), (
            f"V cache mismatch for seq {seq_idx}!"
        )
...
def CHECK_LOADING(
    ...
    try:
        ...
        k_cache_check, v_cache_check = (
            torch.cat(cat_k_list, dim=0),
            torch.cat(cat_v_list, dim=0),
        )
