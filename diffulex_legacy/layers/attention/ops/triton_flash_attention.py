...
class _attention(torch.autograd.Function):
    ...
    def forward(
        ...
        if block_table is not None:
            raise NotImplementedError("Prefix Caching is not supported in this version, block_table can only be None.")
        ...
        if isinstance(p_scale, torch.Tensor):
            if p_scale.numel() != 1:
                raise ValueError(f"Expected p_scale to be a scalar tensor (numel==1), got shape={tuple(p_scale.shape)}")
