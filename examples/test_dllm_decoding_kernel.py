...
from diffulex_legacy.layers.attention.ops import diffusion_lm_parallel_flash_decoding
...
if __name__ == "__main__":
    ...
    build_up_cu_seqlens = (
        lambda x: torch.tensor([0] + list(torch.cumsum(x, dim=0).cpu().numpy())).to(torch.int32).to("cuda")
    )
