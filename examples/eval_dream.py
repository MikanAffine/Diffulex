...
class DreamLoRA(LM):
    ...
    def _generate_batch(self, prompts: List[str]) -> List[str]:
        ...
        for i, prompt in enumerate(prompts):
            ...
            if len(prompt_ids) > self.max_length - self.max_new_tokens:
                eval_logger.warning(
                    f"Prompt length {len(prompt_ids)} is larger than {self.max_length - self.max_new_tokens}, cutoff on the left side"
                )
    ...
    def _generate_block_single(self, prompt):
                ...
                if len(block_states) - 1 < (self.max_new_tokens // block_size) and not eos_detected:
                    ...
                    if current_progress >= block_add_threshold:
                        ...
                        x_t = torch.cat(
                            [
                                x_t,
                                torch.tensor([[mask_id] * block_size]).to(self.device),
                            ],
                            dim=1,
                        )
                    ...
                    if active_blocks:
                        ...
                        for bid in active_blocks:
                            if block_states[bid]["start_pos"] >= cache_length:
                                earliest_active_after_cache = min(
                                    earliest_active_after_cache,
                                    block_states[bid]["start_pos"],
                                )
                        ...
                        if earliest_active_after_cache < float("inf"):
                            ...
                            if cache_length >= x_t.shape[1]:
                                print(
                                    f"Cache length ({cache_length}) >= sequence length ({x_t.shape[1]}) at step {step}. Exiting generation loop."
                                )
        ...
        non_eos_tokens = self._count_non_eos_tokens_before_truncation(x_t[0].tolist(), prompt.shape[1])
