...
def test_generation(
    ...
    if not warmup:
        ...
        print("运行 warmup 生成（kernel 编译中，不报告速度）...")
...
示例用法:
  ...
  %(prog)s --gptq --model-path /path/to/quantized/model  # 使用量化后的模型路径
    ...
    strategy_group.add_argument(
        "--gptq",
        action="store_true",
        help="只运行 GPTQ W4A16 相关策略（需要先运行量化脚本生成离线权重）",
    )
    strategy_group.add_argument(
        "--awq",
        action="store_true",
        help="只运行 AWQ W4A16 相关策略（需要先运行量化脚本生成离线权重）",
    )
    strategy_group.add_argument(
        "--strategies",
        type=str,
        help="手动指定策略（逗号分隔），例如: bf16_bf16kv,w8a16_fp8kv",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.3,
        help="GPU 内存利用率（默认: 0.3）",
    )
    parser.add_argument(
        "--no-isolate",
        action="store_true",
        help="多策略运行时不使用子进程隔离（调试用，可能导致状态串扰/性能波动）",
    )
    ...
    w4a16_strategies = [
        s for s in strategies if "w4a16" in s.lower() and "gptq" not in s.lower() and "awq" not in s.lower()
    ]
    ...
    other_strategies = [
        s
        for s in strategies
        if "w4a16" not in s.lower() and "w4a8" not in s.lower() and "gptq" not in s.lower() and "awq" not in s.lower()
    ]
