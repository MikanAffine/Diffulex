#!/usr/bin/env python3
"""测试FP8 KV cache distinct layout：使用FP8存储，在Python层显式反量化为BF16，然后用原来的BF16算子"""
...
def main():
    ...
    print("=" * 60)
    print("初始化 Diffulex 模型 (FP8 KV Cache, Distinct Layout)...")
    print("=" * 60)
    ...
    llm = Diffulex(
        ...
        kv_cache_layout="distinct",  # 使用 distinct layout
        kv_cache_dtype="fp8_e4m3",  # 使用FP8 KV cache
        decoding_strategy="d2f",
    )

    print("✓ 模型初始化完成 (FP8 KV Cache, Distinct Layout)\n")
    ...
    print("使用FP8 KV cache (distinct layout)，Python层反量化为BF16后使用BF16算子")
    print("=" * 60)
    ...
    print("\n✓ FP8 KV Cache (Distinct Layout, Python层反量化) 端到端测试完成！")
