...
def _chunked_prefill_attn_unified_kernel(
    ...
    q_block_end = q_block_start + BLOCK_M
    ...
    if IS_BLOCK_CAUSAL and not IS_PREFIX_FULL:
        ...
        if is_prefilling:
            prefix_range = tl.cdiv(tl.minimum(prefix_len, valid_kv_len), BLOCK_N)
            padded_prefix_range = tl.cdiv(tl.minimum(padded_prefix_len, valid_kv_len), BLOCK_N)
            mixed_prefill_len = tl.maximum(padded_prefix_len, max_rel_kv_len)
            mixed_prefill_range = tl.minimum(tl.maximum(0, tl.cdiv(mixed_prefill_len, BLOCK_N)), full_range)
            if q_block_end <= prefix_len:
                loop_range = prefix_range
            elif q_block_start >= prefix_len:
                if q_block_end <= padded_prefix_len:
                    loop_range = padded_prefix_range
                elif q_block_start >= padded_prefix_len:
                    loop_range = block_causal_range
                else:
                    loop_range = mixed_prefill_range
            else:
                loop_range = mixed_prefill_range
        else:
            loop_range = block_causal_range
    ...
    for kv_block_id in range(0, loop_range):
        ...
        if IS_BLOCK_CAUSAL and not IS_PREFIX_FULL:
            ...
            if is_prefilling:
                if q_block_end <= prefix_len:
                    score_mask = score_valid_mask & (offs_kv_block < prefix_len)[None, :]
                elif q_block_start >= prefix_len:
                    if q_block_end <= padded_prefix_len:
                        score_mask = score_valid_mask & (offs_kv_block < padded_prefix_len)[None, :]
                    elif q_block_start >= padded_prefix_len:
                        score_block_mask = ((abs_q_block // DLLM_BLOCK_SIZE + 1) * DLLM_BLOCK_SIZE)[
                            :, None
                        ] > abs_kv_block[None, :]
                        score_mask = score_valid_mask & score_block_mask
                    else:
                        score_pure_prefix_mask = (offs_q_block < prefix_len)[:, None] & (offs_kv_block < prefix_len)[
                            None, :
                        ]
                        ...
                        score_mask = score_valid_mask & (
                            score_pure_prefix_mask | score_padded_causal_mask | score_block_mask_extend_only
                        )
                else:
