...
def _store_kv_cache_distinct_fp8(
    key: torch.Tensor,
    value: torch.Tensor,
    ...
    *,
    ...
    dllm_store_kv_cache_kernel_distinct_fp8[GRID](
        ...
        v_cache,
        slot_mapping,
        key_quantized.stride(0),
        value_quantized.stride(0),
        *k_cache.stride(),
        *v_cache.stride(),
        NHeads,
        HDim,
...
def _store_kv_cache_distinct_bf16(
    key: torch.Tensor,
    value: torch.Tensor,
    k_cache: torch.Tensor,
    v_cache: torch.Tensor,
...
def _store_kv_cache_unified_fp8(
    key: torch.Tensor,
    value: torch.Tensor,
    k_cache: torch.Tensor,
    v_cache: torch.Tensor,
...
def _store_kv_cache_unified_bf16(
    key: torch.Tensor,
    value: torch.Tensor,
    k_cache: torch.Tensor,
    v_cache: torch.Tensor,
    ...
    if is_unified:
        ...
        if os.getenv("DIFFULEX_DEBUG_FP8_LOAD_REF", "0") == "1":
            ...
            if 0 <= total_tokens <= 4096:
                ...
                print(
                    f"[DIFFULEX_DEBUG_FP8_LOAD_REF] max_abs_diff k={max_diff_k:.6g} v={max_diff_v:.6g} (total_tokens={total_tokens})"
                )
